...
signal disable_attack_changed(bool)
...
var disable_attack_stack := 0
@export var disable_attack := false:
	set(v):
		if v: disable_attack_stack += 1
		else: disable_attack_stack = max(disable_attack_stack - 1, 0)
		var disabled := disable_attack_stack > 0
		if v != disabled:
			disable_attack = v
			disable_attack_changed.emit(v)
